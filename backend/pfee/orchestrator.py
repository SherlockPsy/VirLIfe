...
from backend.persistence.models import WorldModel, AgentModel, ArcModel, IntentionModel, MemoryModel
...
class PerceptionOrchestrator:
    ...
    async def run_perception_cycle(
        ...
        # Environmental triggers - detect environmental shifts
        ...
        for entity in entities:
            ...
            if entity.get("type") in ["location", "place"]:
                entity_type = EntityType.LOCATION
            elif entity.get("type") in ["object", "item"]:
                entity_type = EntityType.OBJECT
            elif entity.get("type") in ["organisation", "institution"]:
                entity_type = EntityType.ORGANISATION
            elif entity.get("type") in ["information_source", "contact"]:
                entity_type = EntityType.INFORMATION_SOURCE
        ...
        try:
            ...
            # Extract structured physical changes from renderer if available
            # Renderer output should include structured data about physical changes
            physical_changes = {}
            if hasattr(self.render_engine, 'last_render_output'):
                last_output = getattr(self.render_engine, 'last_render_output', {})
                physical_changes = last_output.get("physical_changes", {})
    ...
    def _extract_current_context(self, world_state: Dict[str, Any]) -> Dict[str, Any]:
        ...
        return {
            ...
            "agents_present": world_state.get("agents_present", [])
        }
    ...
    async def _build_cognition_input(
        ...
        """Build complete cognition input packet."""
        ...
        from backend.persistence.models import ArcModel, IntentionModel, MemoryModel
        ...
        event_triviality = EventTrivialityClassification.NON_TRIVIAL  # Agent initiative is non-trivial

        # Determine behavioral choices
        from backend.cognition.eligibility import BehavioralChoice
        behavioral_choices = [
        ...
        # Compute personality activation from Personality Compiler
        from backend.personality.compiler import PersonalityCompiler
        
        personality_activation = {}
        if agent.personality_kernel:
            # Use Personality Compiler to compute activation packet
            # This is a deterministic computation based on kernel + current state
            ...
            # Compute activation deterministically
            # Activation reflects how current state modulates stable personality
            drives_dict = {
                k: (v.get("level", 0.0) if isinstance(v, dict) else v)
                for k, v in (agent.drives or {}).items()
            }
            
            # Weighted combination of state factors
            activation_stress = sum(drives_dict.values()) / max(len(drives_dict), 1) if drives_dict else 0.0
            activation_arousal = mood_arousal
            activation_valence = mood_valence
            
            personality_activation = {
                "stress_modulation": activation_stress,
                "arousal_modulation": activation_arousal,
                "valence_modulation": activation_valence,
        ...
        return CognitionInput(
            ...
            event_time=world_state.get("current_time", datetime.now(datetime.timezone.utc)),
            ...
            mood=(
                agent.mood.get("valence", 0.0) if isinstance(agent.mood, dict) else 0.0,
                agent.mood.get("arousal", 0.0) if isinstance(agent.mood, dict) else 0.0
            ),
            drives={
                k: (v.get("level", 0.0) if isinstance(v, dict) else v)
                for k, v in (agent.drives or {}).items()
    ...
    async def _build_renderer_input(
        ...
        # Build comprehensive renderer context
        renderer_input = {
            ...
            "context": context,
            "resolved_potentials_count": len(context.get("resolved_potentials", [])),
    ...
    async def _detect_environmental_shifts(
        ...
        if current_location:
            # Check if location atmosphere has changed
            # Check location attributes for environmental shift potential
            location_type = world_state.get("current_location_type", "general")
            location_name = world_state.get("current_location", "")
            
            # Locations with higher environmental activity
            ...
            # Check for weather/time-based shifts (if tracked in world state)
        ...
        # Check for time-based shifts (e.g., day/night transitions)
        current_time = world_state.get("current_time")
        if current_time:
