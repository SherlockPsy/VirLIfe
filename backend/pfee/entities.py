...
class EntityPersistenceManager:
    ...
    async def classify_entity_persistence(
        ...
        try:
            ...
            logger.error(f"Error classifying entity persistence: {str(e)}", exc_info=True)
            # Default to ephemeral on error
            return PersistenceLevel.EPHEMERAL
    ...
    async def promote_to_persistent(
        ...
        if entity_type == EntityType.PERSON:
            # For agents, we might store this in a separate table or JSON field
            # For now, we'll use a status_flags JSON field if it exists
            if hasattr(entity, "status_flags"):
                ...
                # For non-agent entities, we need to track persistence differently
                # Create a separate persistence tracking mechanism
                # For now, store in entity metadata or create separate tracking
                from sqlalchemy import select, update
                from backend.persistence.models import ObjectModel, LocationModel
    ...
    def _is_core_person(self, entity: Dict[str, Any]) -> bool:
        ...
        # Check relationship metadata or explicit flags
    ...
    def _has_long_term_obligations_with_user(
        ...
        # Check calendar items or obligations
